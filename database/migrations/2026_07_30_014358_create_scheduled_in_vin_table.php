...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('scheduled_in_vin', function (Blueprint $table) {
            ...
            $table->increments('id');
            $table->date('scheduled_in_date')->nullable()->comment('Scheduled In Date');
            ...
            $table->float('estimate_amount')->nullable()->default(0)->comment('Estimate amount the vehicle repair would cost.');
            $table->boolean('total_loss')->nullable()->default(false);
            $table->string('shop_name', 60)->nullable()->comment('Shop Location');
            $table->unsignedSmallInteger('shop_id')->nullable()->default(0)->comment('Location ID set depending on the value in Location_ID table.');
        });
    }
